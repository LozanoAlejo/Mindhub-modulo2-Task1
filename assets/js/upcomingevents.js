...
.then(data=>{ 
    fetchHome = data
    upcomingg = fetchHome.events.filter( upcom => upcom.date >= fetchHome.currentDate)
    renderTemplate (crearCards(upcomingg), ubicacion)
...
function filtroCruzado(evento){
    ...
    const filterPerFind = searchmovie (search, upcomingg)
